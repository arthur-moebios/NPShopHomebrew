...
private:
    ...
    static constexpr inline fs::FsPath JSON_PATH{"/switch/npshop/cache/cache.json"};
...
void GetDownloadTempPath(fs::FsPath& buf) {
    ...
    std::snprintf(buf, sizeof(buf), "/switch/npshop/cache/download_temp%lu", count_copy);
}
...
void SetCommonCurlOptions(CURL* curl, const Api& e) {
    ...
    CURL_EASY_SETOPT_LOG(curl, CURLOPT_FAILONERROR, (e.GetFlags() & Flag_AllowErrorBody) ? 0L : 1L);
...
// --- helper local para inserir/normalizar "Key: Value" em curl::Header ---
static inline void header_insert_line(Header& h, const std::string& line) {
    auto pos = line.find(':');
    ...
    // trim simples do começo de 'val'
    size_t p = val.find_first_not_of(" \t\r\n");
    if (p != std::string::npos) val.erase(0, p);

    if (!key.empty() && !val.empty()) {
        h.m_map.insert_or_assign(key, val);
    }
}

bool post_json_with_headers_curl(const char* url,
    ...
    long& http_code)
{
    ...
    // Monta o header base (Content-Type/Accept) + extras
    Header hdr;
    // se o usuário passar Content-Type próprio, não vamos sobrescrever:
    if (hdr.Find("Content-Type") == hdr.m_map.end()) {
        hdr.m_map.emplace("Content-Type", "application/json");
    }
    if (hdr.Find("Accept") == hdr.m_map.end()) {
        hdr.m_map.emplace("Accept", "application/json");
    }
    for (const auto& line : headers) {
        header_insert_line(hdr, line);
    }

    // Executa POST com corpo JSON em memória
    ApiResult res = Api().ToMemory(
        Url{ url ? url : "" },
        Fields{ json },
        std::move(hdr)
    );
    ...
    // success aqui já considera libcurl OK; garantimos 2xx também:
    const bool ok = res.success && http_code >= 200 && http_code < 300;
    return ok;
}
